...
import {defaultUserInfo, UserInfo} from "@/types/userInfo";
...
const DropdownUser = () => {
    ...
    useEffect(() => {
        function fetchUserInfo(): void {
            let userInfoJson: any = localStorage.getItem("userInfo");
            try {
                // Check localStorage first for faster retrieval
                if (userInfoJson) {
                    userInfoJson = JSON.parse(userInfoJson);
                    setUserInfo(userInfoJson);
                    return; // Exit the function if found in localStorage
                }

                // Handle Observable from userService.getUserInfo()
                const userInfoObservable = userService.getUserInfo();
                ...
                const subscription = userInfoObservable.subscribe({
                    ...
                    error: err => {
                        ...
                        setUserInfo(defaultUserInfo); // Set to default if Observable errors
                    },
                    complete: () => {
                        // Optional: Perform actions after Observable completes (e.g., cleanup)
                        subscription.unsubscribe(); // Unsubscribe to prevent memory leaks
                    }
                });
            } catch (err) {
                console.error('Failed to get user info (initial attempt):', err);
                setUserInfo(defaultUserInfo); // Set to default if initial attempt fails
            }
        }
